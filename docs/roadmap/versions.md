...
以下依据现行文档与已有评测记录整理（2026-09-22），不是本次重新运行测试后的认证。状态使用“规划、实现中、待验收、已验收”；阶段状态以全部必需交付的证据为准。

| 范围 | 状态 | 依据与缺口 |
|------|------|------------|
| V1.0–V1.3 | 待验收 | [现行架构](../architecture/architecture.md) 描述主要组件已存在；需汇总对应自动测试与端到端结果 |
| V1.4 | 实现中，待补验收证据 | 保留至少 20 条生成样例、20 条标注检索查询及完整闭环要求；现有文档记录 RAG 查询集缺失 |
| V2.1–V2.3 | 实现中，部分能力前置 | [评测索引](../evaluations/README.md) 已有 v0.2 与 v9 实验；v9 的 l01 结构和加工未合格，不能据此宣布 V2 达标 |
