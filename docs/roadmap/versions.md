...
| 范围 | 状态 | 依据与缺口 |
|------|------|------------|
| V1.0–V1.3 | 已验收（2026-09-26） | [现行架构](../architecture/architecture.md) 描述的主要组件已存在；自动测试与闭环结果的汇总见 [v1-acceptance-report.md](../evaluations/v1-acceptance-report.md) §3（`pytest tests` 478 passed）与 §9（用户手动功能验收，覆盖输入→审批→后续检索闭环） |
| V1.4 | 已验收（2026-09-26，tag `v1.0.0`） | **RAG 查询集缺口已闭合**：40 条带证据标注的查询（dev 24 / holdout 16）+ 20 条 Agent 场景、可重复的评测入口与真实基线（[rag-v1-report.md](../evaluations/rag-v1-report.md)，2026-09-25）；**生成样例已补齐并实跑**：25 条 g01–g25 行为门与内容审查 25/25、自动测试 478 passed（[v1-acceptance-report.md](../evaluations/v1-acceptance-report.md)，2026-09-26）；手动功能验收由用户执行并反馈通过（报告 §9，未逐项留证） |
...
V1 已验收（2026-09-26，tag `v1.0.0`）：V1.0–V1.4 的主要组件已存在，RAG 查询集缺口已在 [rag-v1-report.md](../evaluations/rag-v1-report.md)（2026-09-25）闭合，生成样例的实跑与逐条审查见 [v1-acceptance-report.md](../evaluations/v1-acceptance-report.md)（2026-09-26），手动功能验收由用户执行并反馈通过。后续质量加固进入 V2。
