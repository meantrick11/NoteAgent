...
以下项目**由用户执行**，本报告不代其判定；状态一律为"待用户测试"，未收到反馈前不标记为通过。

| # | 场景 | 操作 | 期望 | 状态 |
|---|------|------|------|------|
| 1 | 创建后跨会话检索 | 新建会话记一条新笔记并审批通过，再开另一个会话提问该主题 | 能检索到刚写入的笔记，引用可溯源 | 待用户测试 |
| 2 | 追加 | 对已有笔记提出追加内容并审批 | 原文保留，新内容进入文件末尾 | 待用户测试 |
| 3 | 覆盖 | 对已有笔记提出更正并审批（replace） | 过时内容被替换，一级标题与其他章节保留 | 待用户测试 |
| 4 | 删除 | 对某篇笔记提出删除并审批 | 文件被删除，向量同步移除 | 待用户测试 |
| 5 | 拒绝 | 提出提案后点拒绝 | 文件不变，不产生索引变更 | 待用户测试 |
| 6 | 重启恢复 | 保留一份未审批草稿后重启服务，重新打开该会话 | 历史消息与待审草稿卡片都能恢复 | 待用户测试 |
| 7（可选） | 写盘失败 | 让目标文件不可写（例如占用或只读目录）后审批 | 返回错误，文件未变，**待审草稿仍在**，恢复可写后可再次审批成功 | 待用户测试 |

第 7 项对应本轮修复；自动测试已用注入异常覆盖，真实文件系统上的表现仍需人工确认。
...
- **未完成项**：① 手动功能验收（§9）待用户执行并反馈；② 旧集 `b05` 为不稳定样例，模型在冲突分支下可能"只说明不提案"并宣称已提案；③ L1 的 `structure.heading_precision`、`faithful.hedge`、`form.list`、`file_name_topic` 偏差未修（计划未授权改打分器）；④ 对话类锚点设计过严，下一轮需调整输入或只放进审查清单。
- **不宣布**：V1 整体验收完成。当前位置是"本轮修复 + 自动化 + 生成评测的实际完成状态，手动功能待验"。路线图中的 V1 状态据此更新为"待验收（剩手动功能验收）"。
