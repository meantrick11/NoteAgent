...
- 准备四类固定候选：
  ...
  3. 可读但遗漏严重的摘要；
  4. 含无来源结论的“丰富笔记”。
- 固定排序契约：优秀笔记 > 机械译文 > 遗漏摘要；幻觉候选必须被忠实硬门淘汰。优秀与机械译文都应达到流畅阈值；优秀靠结构和加工增益领先，流畅度不是知识加工的区分轴。
...
- 优秀候选：通过全部硬门，结构、流畅、加工增益均不低于 3/4；结构、加工增益严格高于机械译文，维度总和亦高于机械译文。
- 当前逐段译文：通过忠实与完整，流畅亦应达标，但因加工增益（及/或结构）不足不得整体合格。
- 遗漏摘要：完整硬门失败。
- 幻觉候选：忠实硬门失败。
...
同日用 deepseek-v4-flash 对四个固定候选做 Judge 校准（`judge_independent=false`，生成与 Judge 同模型）。Judge 给 `good.md` 与 `literal.md` 的 fluent 均为 4，其它契约（硬门、复习题、结构/加工严格领先、维度总和）均通过；仅旧的 `good_fluent_gt_literal` 严格比较失败。证据表明机械译文可以同样流畅，流畅度不应作为优秀相对机械译文的区分轴。校准契约改为 `good_fluent_at_least_threshold` 与 `literal_fluent_at_least_threshold`（阈值取 `quality_thresholds.fluent`，默认 3），保留结构、加工与维度总和的严格领先要求。未修改 Judge 打分或 fixtures 去迎合契约。
