...
正式架构见 [docs/architecture/knowledge-workflow-v1.md](docs/architecture/knowledge-workflow-v1.md)。尚未做 Job 状态机、审批后自动索引。
...
聊天：`POST /chat`（SSE）。侧栏可切换、重命名、删除历史（`PATCH`/`DELETE /conversations/{id}`）。有笔记提案时前端出卡片，`POST /chat/review` 同意后才写盘。
...
4. 迁移建表：`uv run alembic upgrade head`
