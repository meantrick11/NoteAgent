...
进程日志和 LangChain 回调追踪。不记 API key、不把整份网页/笔记正文打进 INFO。

## 包含模块

| 文件 | 模块 | 作用 |
|------|------|------|
| `logging.py` | `setup_logging`、`ColoredFormatter` | 彩色 stdout + `log_dir/noteagent.log` 轮转 |
| `agent_trace.py` | `AgentTraceHandler` | LLM/工具 start、end、error、耗时、截断回复 |
| `__init__.py` | 再导出 | `from noteagent.observability import setup_logging` |
