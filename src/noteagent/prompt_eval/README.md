...
| 文件 | 作用 |
|------|------|
| `cases.py` | 读 `cases.jsonl` / `learning_notes.jsonl`；按 `task_mode` 选 v0.1 或 v0.2 |
| `score.py` | 旧 case 的 v0.1 分数 + 学习型笔记硬门和维度资格判定 |
| `judge.py` | 独立调用语义 Judge，严格校验 JSON；契约失败最多重试 3 次 |
| `calibration.py` | 四个固定候选的校准契约 |
...
`--judge` 优先用 `JUDGE_MODEL`；为空则警告并回退到 `CHAT_MODEL`，配置记 `judge_independent=false`。模型名可与 `CHAT_MODEL` 相同，同样记非独立。不传 `--judge` 时保持旧流程，学习型 case 明确显示“语义评测未完成”，不生成语义总分。Judge 响应缺字段、分数越界、证据不是原文/草稿逐字片段、或不是严格 JSON 时记入 `judge_failures`，不会静默补默认值。
