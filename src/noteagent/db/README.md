...
| 文件 | 模块 | 作用 |
|------|------|------|
| `models.py` | `Base`、`Conversation`、`Message` | 表结构（列名、索引、FK CASCADE）；含上下文列 `running_summary`、`summary_watermark_turn_id`、`pending_draft` 与 `Message` 的 `turn_id` / tool stub / `citations` |
...
生产库用 PostgreSQL + Alembic（现行 head `a9b4c2d1e8f0`）。单测用内存 SQLite `Base.metadata.create_all`。业务写入口是 `ConversationStore`，不在本包。
