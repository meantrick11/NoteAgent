...
SSE：先 `event: conversation`（`{id, title}`），可选 `event: sources`（本条助手消息实际引用，编号 1..n），再 `event: token` 拼进助手气泡、流结束 `event: answer` 换成净化并重排后的全文、`event: draft` 打开右侧引用面板的草稿模式（正文可编辑、保存只写待审状态）。`[[cite:N]]` 绘成该条消息内的 ①。点 ① 打开可编辑侧栏，保存走 `PUT /notes/{path}`。切会话时侧栏按会话快照，不把 A 的展开顶到 B。后端内部的 `assistant_final` 不直接推给页面（路由转成 `answer`）。
